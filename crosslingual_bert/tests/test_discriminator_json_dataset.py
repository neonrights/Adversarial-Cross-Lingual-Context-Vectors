...
D_data = DiscriminatorDataset('test_discriminator.txt', vocab, {'en': 0, 'cz': 1}, seq_len=64)
...
for input_ids, labels in D_loader:
	assert input_ids.shape == (32, 64), "got {} for input shape instead of (32, 256)".format(inputs_ids.shape)
	assert labels.shape == (32,), "got {} for label shape instead of (32,)".format(labels.shape)
	break

print("passed on memory batch tests")

D_data = DiscriminatorDataset('test_discriminator.txt', vocab, {'en': 0, 'cz': 1}, seq_len=64, on_memory=False)
D_loader = DataLoader(D_data, batch_size=32, shuffle=True)
