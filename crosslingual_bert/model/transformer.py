...
class SelfAttention(nn.Module):
    ...
    def forward(self, encoder_states, decoder_states, attention_mask):
        mixed_query_layer = self.query(decoder_states)
        mixed_key_layer = self.key(encoder_states)
        mixed_value_layer = self.value(encoder_states)
...
class Attention(nn.Module):
    ...
    def forward(self, encoder_tensor, decoder_tensor, attention_mask):
        self_output = self.self(encoder_tensor, decoder_tensor, attention_mask)
        attention_output = self.output(self_output, decoder_tensor)
...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_states, decoder_states, attention_mask, output_all_encoded_layers=True):
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(encoder_states, decoder_states, attention_mask)
            if output_all_encoded_layers:
                all_encoder_layers.append(hidden_states)
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
